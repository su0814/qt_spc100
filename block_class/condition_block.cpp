    ...
    : QGraphicsRectItem(0, 0, defaultWidth, defaultHeight, parent)
{
    ...
    connect_block* condition_point
        = new connect_block(defaultWidth, defaultHeight / 4, CONNECT_POINT_TYPE_OUTPUT, 0,
    ...
    : QGraphicsRectItem(parent)
{
    ...
    setPos(x, y);
    block_info_init();
    connect_block* condition_point = new connect_block(defaultWidth, defaultHeight / 4, CONNECT_POINT_TYPE_OUTPUT, 0, &block_attribute, this);
...
void condition_block::block_info_init()
{
    ...
    case TOOL_TYPE_CONDI_DI:
        if (condition_di_set.is_reverse) { // DI根据“反向”参数决定代码内容
            block_attribute.logic_string = "(" + func_list[block_attribute.block_info.tool_type][block_attribute.block_info.tool_id]
                + " == false)";
        } else {
            block_attribute.logic_string = "(" + func_list[block_attribute.block_info.tool_type][block_attribute.block_info.tool_id] + " == true)";
        }
    ...
    case TOOL_TYPE_CONDI_QEP:
        block_attribute.logic_string = "((" + lua_qep_dir_func[block_attribute.block_info.tool_id]
            ...
            + calc_str[condition_ai_pi_qep_set.calc_type_index] + QString::number(condition_ai_pi_qep_set.value) + ")";
        param_label->setPlainText(calc_str[condition_ai_pi_qep_set.calc_type_index] + " "
            + QString::number(condition_ai_pi_qep_set.dir) + "-"
            + QString::number(condition_ai_pi_qep_set.value));
        break;
    ...
    QObject::connect(&okButton, &QPushButton::clicked, [&]() {
        ...
        case TOOL_TYPE_CONDI_QEP:
            ...
            block_attribute.logic_string = "((" + lua_qep_dir_func[block_attribute.block_info.tool_id]
                ...
                + func_list[block_attribute.block_info.tool_type][block_attribute.block_info.tool_id]
                + calc_type.currentText() + " " + QString::number(value.value()) + " ))";
            param_label->setPlainText(calc_type.currentText() + " " + QString::number(condition_ai_pi_qep_set.dir) + "-"
